...
class PanoramaProcessor:
    ...
    def verify_2(self, sig: PEMINSignal, new_off_db: float) -> bool:
        """
        Тест ВЫКЛ (п. 7.4 РЭ).
        Если уровень в точке при выключенном тесте превышает исходный шум 
        более чем на порог (или просто стал высоким) -> внешнее устройство.
        
        Логика РЭ: "Критерием для отбраковки... является наличие сигнала".
        Обычно проверяют, что уровень вернулся к шуму. 
        Здесь используем простую проверку: уровень OFF не должен сильно вырасти.
        """
        # Если новый уровень шума значительно выше старого (например, включился вентилятор)
        # то точка считается загрязненной.
