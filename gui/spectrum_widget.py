...
def _marker_color(sig):
    ...
    V1 не запускался:                жёлтый  (255, 220, 50)
    V1 пройден, V2 не запускался:    жёлтый  (255, 220, 50)
    V1 + V2 пройдены (ПЭМИН):        зелёный (50, 220, 80)
    V1 пройден, V2 провален (внеш.): синий   (80, 160, 255)
    V1 провален:                      None — не рисуем
    """
    v1 = sig.verified_1
    v2 = sig.verified_2

    if v1 is None:
        return (255, 220, 50)   # ещё не верифицирован

    if v1 and v2 is None:
        return (255, 220, 50)   # V1 OK, ждём V2

    if v1 and v2:
        return (50, 220, 80)    # ПЭМИН

    if v1 and not v2:
        return (80, 160, 255)   # внешний сигнал (V1 OK, V2 fail)

    return None                  # V1 провален — не рисуем